...
export const CrossoverRateControl: React.FunctionComponent<
  React.HTMLAttributes<HTMLDivElement>
> = ({ className }) => {
  const { configuration, dispatch } = useEvolution();
  const [value, setValue] = useState(configuration.crossoverRate);
  ...
  const handleCommit = (
    ...
    dispatch({
      ...
      payload: {
        configuration: {
          ...configuration,
          crossoverRate: newValue as number,
        },
      },
    });
  };
  ...
  return (
    <Box className={`${className ? className : ""} ${classes.root}`}>
      <Typography>Crossover rate</Typography>
      <Slider
        value={value}
